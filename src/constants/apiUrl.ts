export const SIGNIN_URL = '/api/v2/user/login';
export const SIGNUP_URL = '/api/v2/user/register';
export const LOGOUT_URL = '/api/v2/user/logout';
export const TEST_URL = '/api/v2/user/test';

export const DECODE_PROMPT_URL = '/api/v2/copyright';
export const REGISTER_COPYRIGHT_URL = '/api/v2/copyright/register';

export const PRODUCT_LIST_URL = '/api/v2/product';
export const REGISTER_PRODUCT_URL = '/api/v2/product';
export const BUY_COPYRIGHT_URL = '/api/v2/product/copyright';
export const BUY_TICKET_URL = '/api/v2/product/ticket';
